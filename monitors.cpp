...
//  painter.restore();
//}
 void monitors::set_version(std::string const& v) {
     m_version=v;
     ui.lineEdit_5->setText(v.c_str());
...
monitors::monitors(mainwindow* parent,QWidget* sub):
  ...
  ui.lineEdit_5->setText(m_version.c_str());
  ...
  */
  ...
  m_pump_switch = new SwitchButton(ui.widget_5);
  m_pump_switch->setGeometry(0,0,50,29);
  connect(m_pump_switch,SIGNAL(statusChanged(bool)),this,SLOT(set_pump_status(bool)));
...
void monitors::save_as(){
  ...
  m_fname = fileName.toStdString();
  //std::cout<<"m_fname: "<<m_fname<<std::endl;
}
...
void monitors::set_pump_status(bool status){
  if (!_is_connect){ log(1,"Please connect first"); return; }
  if (modbus_write_bit(m_modbus_context,COIL_ONOFF_PUMP,status)==1){
    std::stringstream sstr; sstr<<"switch pump to "<<(status ? "open" : "close")<<" ok";
    log(0,sstr.str());
    m_leds[2]->status(status ?  simple_led::e_status::k_on : simple_led::e_status::k_off);
  }else{
    m_pump_switch->switch_to(m_data_frame.device_coil[1]);
    log(2,"switch pump failed");
  }
}
...
void monitors::start(){
  ...
  if (!m_fout.is_open()){
    ...
    const size_t as = std::extent<decltype(util::item_names)>::value;
    for (size_t i=0; i<as-1; ++i) m_fout<<util::item_names[i]<<",";
    m_fout<<util::item_names[as-1]<<"\n";
  }
  //for (int i=0; i<10; ++i) m_fout<<m_data_frame<<std::endl;
  //m_fout.close(); return;
  if (_is_fs){
    m_start = std::chrono::system_clock::now();
    _is_fs = false;
...
void monitors::get(){
  if(!_is_connect) {log(1,"connect failed"); return;}
  m_data_frame.rw_lock.lockForWrite();
  int ec0 = modbus_read_registers(m_modbus_context
      ,0,REG_END,&m_data_frame.data[0]);
  int ec1 = modbus_read_bits(m_modbus_context
...
std::ostream& operator<<(std::ostream& os, monitors::data_frame_t& df){
  std::string time=util::time_to_str_s();
  os<<time<<",";
  uint32_t seconds = 0;
  meta::encode(seconds,util::u162u32_indx,df.data[REG_HEARTBEAT_HI],df.data[REG_HEARTBEAT_LW]);
  os<<seconds<<",";
  float yt = util::get_bmp280_1(df.data+REG_TEMPERATURE_DEC);
  float pt = util::get_bmp280_1(df.data+REG_PRESSURE_DEC);
  float pc = util::to_float(df.data+REG_MFC_RATE_PV);
  os<<yt<<",";
  os<<pt<<",";
  os<<pc<<",";
  uint16_t psv = df.data[REG_PUMP_SPEED_SV];
  os<<psv<<",";
  uint16_t mode = df.data[REG_SET_MODE];
  os<<mode<<",";
  os<<df.data[REG_FLOW_DURATION]<<",";
  os<<df.data[REG_RECYCLE_DURATION]<<",";
  os<<(df.device_coil[0] ? 1 : 0)<<",";
  os<<(df.device_coil[1] ? 1 : 0)<<",";
  os<<(df.device_coil[2] ? 1 : 0);
